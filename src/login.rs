...
pub async fn handle_login(
    ...
    match verify_user(&credentials.username, &credentials.password) {
        Ok(true) => {
            ...
            (jar.add(cookie), Redirect::to(&format!("/{}", credentials.username)))
                .into_response()
        }
        Ok(false) => {
            ...
            if current_user == username {
                ...
                if files.is_empty() {
                    ...
                    for file in files {
                        ...
                        html.push_str(&format!("<a href='{}' class='file-link'>{}</a>", file_path, file.name));
                        
                        // Format timestamps
                        let modified = chrono::DateTime::<chrono::Utc>::from(file.modified)
                            .format("%Y-%m-%d %H:%M:%S");
                        
                        html.push_str(&format!("<div class='file-info'>Last modified: {}</div>", modified));
                        html.push_str("</div>");
                    }
                ...
                html.push_str("<a href='/sheet?rows=10&cols=10' class='new-sheet'>Create New Spreadsheet</a>");
