...
fn save_line_graph(
    ...
    let root = BitMapBackend::new(path, (options.width, options.height))
        .into_drawing_area();
    root.fill(&WHITE)?;
        ...
        chart.draw_series(
            data.iter().map(|&(x, y)| {
                let bar = Rectangle::new(
                    [((x as f64 - 0.4) as i32, 0),
                     ((x as f64 + 0.4) as i32, y)],
                    BLUE.filled(),
                );
                bar
            }),
...
fn create_scatter_graph(
    ...
    let mut buffer = vec![0u8; (options.width * options.height * 4) as usize];
    {
        ...
        chart.draw_series(
            data.iter().map(|&(x, y)| 
                Circle::new((x as f64, y as f64), 5, GREEN.filled())
            )
        )?;
...
pub fn create_example_graphs() -> Vec<(String, String)> {
    ...
    let data: Vec<(i32, i32)> = vec![
        (1, 10), (2, 25), (3, 15), (4, 30), (5, 22), (6, 40), (7, 35)
    ];
