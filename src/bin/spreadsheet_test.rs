...
fn test_spreadsheet_evaluate_function() {
    ...
    // Set up some cells with values for testing
    {
        ...
        if let Some(ref mut cell) = sheet.cells[11] {
            // B2
            cell.value = 40;
        }
    }
    ...
    // Test AVG function
    let result = sheet.spreadsheet_evaluate_function("AVG", "A1:B2", "AVG(A1:B2)");
    assert_eq!(result, (25, false)); // (10+20+30+40)/4
    // assert!(!test_cell.error);
    println!("✓ AVG(A1:B2) = {:?} (expected 25)", result);

    // Test MIN function
    let result = sheet.spreadsheet_evaluate_function("MIN", "A1:B2", "MIN(A1:B2)");
    assert_eq!(result, (10, false));
    // assert!(!test_cell.error);
    println!("✓ MIN(A1:B2) = {:?} (expected 10)", result);

    // Test MAX function
    let result = sheet.spreadsheet_evaluate_function("MAX", "A1:B2", "MAX(A1:B2)");
    assert_eq!(result, (40, false));
    // assert!(!test_cell.error);
    println!("✓ MAX(A1:B2) = {:?} (expected 40)", result);

    // Test STDEV function
    let result = sheet.spreadsheet_evaluate_function("STDEV", "A1:B2", "STDEV(A1:B2)");
    ...
    let result = sheet.spreadsheet_evaluate_function("SLEEP", "0", "SLEEP(0)");
    ...
    {
        let result = sheet.spreadsheet_evaluate_function("SLEEP", "A1", "SLEEP(A1)");
...
fn test_dependency_updates() {
    ...
    // Verify that A1 and A2 have "B1" as a dependent.
    {
        ...
        assert!(
            deps_a1.contains(&"B1".to_string()),
            "A1 should have dependent B1"
        );
        println!("✓ A1 contains dependent B1");

        let idx_a2 = get_cell_index(&sheet, 2, 1);
        let deps_a2 = sheet.collect_dependent_keys(sheet.cells[idx_a2].as_ref().unwrap());
        assert!(
            deps_a2.contains(&"B1".to_string()),
            "A2 should have dependent B1"
        );
    ...
    // Verify that A1 no longer has "B1" while A2 still does.
    {
        ...
        assert!(
            !deps_a1.contains(&"B1".to_string()),
            "A1 should not have dependent B1 after update"
        );
        ...
        assert!(
            !deps_a2.contains(&"B1".to_string()),
            "A2 should not have dependent B1 after update"
        );
        println!("✓ After update, A2 does not contains dependent B1");

        let idx_a3 = get_cell_index(&sheet, 3, 1);
        let deps_a3 = sheet.collect_dependent_keys(sheet.cells[idx_a3].as_ref().unwrap());
        assert!(
            deps_a3.contains(&"B1".to_string()),
            "A3 should have dependent B1 after update"
        );
    ...
    for row in 1..=4 {
        ...
        let deps = sheet.collect_dependent_keys(sheet.cells[idx].as_ref().unwrap());
        assert!(
            deps.contains(&"B2".to_string()),
            "A{} should have dependent B2",
            row
        );
    ...
    // Verify that A3 and A4 have "C1" as a dependent.
    {
        let idx_a3 = get_cell_index(&sheet_large, 3, 1);
        let deps_a3 =
            sheet_large.collect_dependent_keys(sheet_large.cells[idx_a3].as_ref().unwrap());
        assert!(
            deps_a3.contains(&"C1".to_string()),
            "A3 should have dependent C1"
        );
        println!("✓ A3 contains dependent C1");

        let idx_a4 = get_cell_index(&sheet_large, 4, 1);
        let deps_a4 =
            sheet_large.collect_dependent_keys(sheet_large.cells[idx_a4].as_ref().unwrap());
        assert!(
            deps_a4.contains(&"C1".to_string()),
            "A4 should have dependent C1"
        );
...
fn test_topo_sort() {
    ...
    for (i, (row, col)) in sorted_cells.iter().enumerate() {
        println!(
            "  {}. {}{}",
            i + 1,
            Spreadsheet::col_to_letter(*col as i32),
            row
        );
    }
    ...
    assert!(
        !sorted_cells.is_empty(),
        "Sorted result should not be empty"
    );
...
fn test_spreadsheet_set_cell_value() {
    ...
    assert!(sheet.cells[index_c3].as_ref().unwrap().error);
