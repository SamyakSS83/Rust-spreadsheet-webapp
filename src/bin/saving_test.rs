...
use cop::saving::{save_spreadsheet, load_spreadsheet};
...
fn test_save_and_load_spreadsheet() -> std::io::Result<()> {
    ...
    let mut sheet = Spreadsheet::spreadsheet_create(5, 5)
    ...
    let c1_index = (0 * sheet.cols + (2)) as usize; // C1 is at row 0, col 2 in 0-indexed
    ...
    // Check cell values
    let a1_index = (0 * loaded_sheet.cols + (0)) as usize; // A1 is at row 0, col 0 in 0-indexed
    let a1_value = loaded_sheet.cells[a1_index].as_ref().unwrap().value;
    assert_eq!(a1_value, 10, "A1 value wasn't preserved");
    
    let b1_index = (0 * loaded_sheet.cols + (1)) as usize; // B1 is at row 0, col 1 in 0-indexed
    ...
    let c1_loaded_index = (0 * loaded_sheet.cols + (2)) as usize; // C1 is at row 0, col 2 in 0-indexed
    ...
    let c1_formula = loaded_sheet.cells[c1_loaded_index].as_ref().unwrap().formula.as_ref().unwrap();
